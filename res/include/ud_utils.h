...
// # define ud_ut_array(type, ...) ({ type *new_arr; type in_val[] = {__VA_ARGS__}; size_t len = sizeof(in_val) / sizeof(type); new_arr = ud_ut_malloc(len * sizeof(type)); type *p_new_arr = new_arr; for (ud_ut_count i = 0; i < len; ++i, ++p_new_arr) *p_new_arr = in_val[i]; new_arr; })
# define ud_ut_array(type, ...) ({ type *new_arr; type in_val[] = {__VA_ARGS__}; type *in_tmp = in_val; size_t len = sizeof(in_val) / sizeof(type); new_arr = ud_ut_malloc(len * sizeof(type)); type *p_new_arr = new_arrfor (ud_ut_count i = 0; i < len; ++i) *p_new_arr++ = *in_tmp++; new_arr; })
/*
